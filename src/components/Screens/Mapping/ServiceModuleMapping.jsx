...
const ServiceModuleMapping = props => {
  ...
  const onServiceChange = async service_id => {
    const response = await getAlreadyMappedModuleServices(
      user.Authorization,
      service_id
    );
    ...
    if (response.data.result.length > 0) {
      for (let i = 0; i < response.data.result.length; i++) {
        tempList.push(response.data.result[i].module_id);
      }
      ...
      <Form onSubmit={onSubmit}>
        ...
        <Form.Item label="Modules">
          {getFieldDecorator('module_id_list', {
            rules: [{ required: true }],
            initialValue: moduleServices
          })(<Modules mode="multiple" onChange={onServiceModuleChange} />)}
        </Form.Item>
