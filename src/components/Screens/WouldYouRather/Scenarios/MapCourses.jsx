...
const ScenarioMapCourses = props => {
      ...
      try {
        ...
        for (let i = 0; i < response.result.wyr_scenario_list.length; i++) {
          if (response.result.wyr_scenario_list[i].id === scenarioId) {
            if (
              response.result.wyr_scenario_list[i].mapped_fm_course.length > 0
            ) {
              for (
                let j = 0;
                j < response.result.wyr_scenario_list[i].mapped_fm_course.length;
                j++
              ) {
                tempList.push(
                  response.result.wyr_scenario_list[i].mapped_fm_course[j]
                    .fm_course_id
                );
              }
            }
...
export default Form.create()(ScenarioMapCourses);
