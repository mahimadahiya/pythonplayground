...
import { mapKeyword, fetchFMCourses } from "../../../../actions";

const MapKeyword = props => {
  ...
  const onSubmit = e => {
    ...
    props.form.validateFields(async err => {
      if (!err) {
        ...
        try {
          await mapKeyword(user.Authorization, data);
      ...
      <Form onSubmit={onSubmit}>
        ...
        {type === "parameter" ? (
          ...
          <Form.Item label="Courses">
            {getFieldDecorator("courses", { rules: [{ required: true }] })(
              <Select
                mode="multiple"
...
export default Form.create()(MapKeyword);
